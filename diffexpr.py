...
def plot_results(dset_list, gene_lists, output_dir, sessionID , cmap="Reds", plot_width=3, dot_size=6):
    ...
    plot_dir = os.path.join(output_dir, f'output_plots_{sessionID}')
...
def plot_results_2(dset_list, gene_lists, output_dir, sessionID, dotSize2 = 6, outlineThickness = 1):
    size_factor = 3.15  # Size factor for height
    ncol = len(dset_list)  # Number of columns = number of datasets
    nrow = 1  # Single row
    ...
    # Create the output directory if it doesn't exist
    plot_dir = os.path.join(output_dir, f'output_plots_2_{sessionID}')
    os.makedirs(plot_dir, exist_ok=True)
...
if __name__=="__main__":
    ...
    if "plot_results" == sys.argv[1] :
        ...
        # Example argument parsing for the `plot_results` function
            ...
            sID = sys.argv[4] 
            customization_params = json.loads(sys.argv[5])  # Customization parameters
            ...
            plot_results(dset_list, gene_lists, '', sID,  cmap=customization_params['cmap'], 
                        plot_width=customization_params['plotWidth'], dot_size=customization_params['dotSize'])
            plot_results_2(dset_list, gene_lists, '' , sID,  dotSize2=customization_params['dotSize2'] , 
    ...
    #gene_lists.append(query)
        ...
        "cmap": "plasma",  # Default values in case customization data is missing
        "plotWidth": 3,
        "dotSize": 9,
        "dotSize2": 6,
        "outlineThickness": 1
